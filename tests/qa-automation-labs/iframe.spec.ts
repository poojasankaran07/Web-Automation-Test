...
test.describe.only("Web Table Flow", () => {
    test("Web Table Flow", async ({ page, menusPage, iframePage }) => {
        ...
        await test.step("Click `Click Me` button in frame 1 and verify the message", async () => {
            ...
            await expect(iframePage.frameMessage).toHaveText('You have clicked on iframe 1 button');
        })
        ...
        await test.step("Click `Click Me` button in frame 2 and verify the message", async () => {
            ...
            await expect(iframePage.frameMessage).toHaveText('You have clicked on iframe 2 button');
        })
