...
test.describe("Radio Button Flow", () => {
  test("Radio Button Flow", async ({ page, menusPage, radioButtonPage }) => {
    ...
    await test.step("Gender Radio button section", async () => {
      await expect(
        radioButtonPage.genderRadioButtonSingle("male"),
      ).not.toBeChecked();
      await expect(
        radioButtonPage.genderRadioButtonSingle("female"),
      ).not.toBeChecked();
      ...
      await expect(radioButtonPage.selectedGender).toHaveText(
        /You selected: Female/,
      );
    ...
    await test.step("Gender and Age section", async () => {
      ...
      await expect(
        radioButtonPage.genderInComboSection("female"),
      ).toBeChecked();
      await expect(
        radioButtonPage.genderInComboSection("male"),
      ).not.toBeChecked();

      await radioButtonPage.ageInComboSection("radio-age-18-35").check();
      await expect(
        radioButtonPage.ageInComboSection("radio-age-18-35"),
      ).toBeChecked();
      await expect(
        radioButtonPage.ageInComboSection("radio-age-35-plus"),
      ).not.toBeChecked();
      await expect(
        radioButtonPage.ageInComboSection("radio-age-under-18"),
      ).not.toBeChecked();
