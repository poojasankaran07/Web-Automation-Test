...
test.describe("Notification Flow", () => {
    test("Notification Flow", async ({ page, menusPage, notificationPage }) => {
        ...
        await test.step("Click all notification buttons and verify the notifications listed", async () => {
            ...
            await expect(notificationPage.notifications.nth(0))
                .toContainText("Notification Body");

            await expect(notificationPage.notifications.nth(1))
                .toContainText("Notification Body");

            await expect(notificationPage.notifications.nth(2))
                .toContainText("Notification Body");

            await expect(notificationPage.notifications.nth(3))
                .toContainText("Notification Body");
        });
    });
});
