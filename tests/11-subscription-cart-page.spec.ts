...
test.describe("Verify Subscription in Cart page", () => {
  test("Verify Subscription in Cart page Flow", async ({
    page,
    homePage,
    subscriptionPage,
  }) => {
    ...
    await test.step(`Click 'Cart' button & Scroll down to footer & verify text 'SUBSCRIPTION'`, async () => {
    ...
    await test.step(`Verify success message 'You have been successfully subscribed!' is visible`, async () => {
      ...
      await expect(subscriptionPage.successfulSubscribeMessage).toHaveText(
